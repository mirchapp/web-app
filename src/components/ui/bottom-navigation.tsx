...
const tabs: Tab[] = [
  {
    id: "home",
    label: "Home",
    icon: { filled: HomeSolidIcon, outline: HomeIcon },
  },
  {
    id: "discover",
    label: "Find",
    icon: { filled: MagnifyingGlassSolidIcon, outline: MagnifyingGlassIcon },
  },
  {
    id: "post",
    label: "Post",
    icon: { filled: PlusCircleSolidIcon, outline: PlusCircleIcon },
  },
  {
    id: "videos",
    label: "Flix",
    icon: { filled: VideoCameraSolidIcon, outline: VideoCameraIcon },
  },
  {
    id: "profile",
    label: "Profile",
    icon: { filled: UserSolidIcon, outline: UserCircleIcon },
  },
];
...
export function BottomNavigation({
  ...
  const tintMap: Record<string, { light: string; glow: string }> = {
    ...
    liked: {
      light: "rgba(252, 165, 165, 0.06)",
      glow: "rgba(248, 113, 113, 0.1)",
    },
    post: {
      light: "rgba(129, 230, 217, 0.10)",
      glow: "rgba(45, 212, 191, 0.16)",
    },
    videos: {
      light: "rgba(196, 181, 253, 0.06)",
      glow: "rgba(167, 139, 250, 0.1)",
    },
    profile: {
      light: "rgba(196, 196, 255, 0.05)",
  ...
  React.useLayoutEffect(() => {
    const compute = () => {
      ...
      if (isDragging && dragX != null) {
        ...
        const left = Math.min(
          Math.max(leftUnclamped, -24),
          containerBounds.width - width + 24
        );
    ...
    const observer = containerElForObserver
      ? new ResizeObserver(() => compute())
      : null;
    if (observer && containerElForObserver)
      observer.observe(containerElForObserver);
    return () => {
  ...
  React.useEffect(() => {
    const detectBackground = () => {
      ...
      if (!hasUserScrolledRef.current) {
        ...
        setIsDarkBackground(
          activeTab === "videos" || activeTab === "post" ? true : themeDark
        );
        return;
      }
    ...
    timeoutIds.push(
      setTimeout(() => requestAnimationFrame(detectBackground), 0)
    );
    timeoutIds.push(
      setTimeout(() => requestAnimationFrame(detectBackground), 16)
    );
    timeoutIds.push(
      setTimeout(() => requestAnimationFrame(detectBackground), 50)
    );
    timeoutIds.push(
      setTimeout(() => requestAnimationFrame(detectBackground), 150)
    );
    timeoutIds.push(
      setTimeout(() => requestAnimationFrame(detectBackground), 300)
    );
  ...
  React.useEffect(() => {
    ...
    setIsDarkBackground(
      activeTab === "videos" || activeTab === "post" ? true : getThemeIsDark()
    );
  }, [activeTab, getThemeIsDark]);
      ...
      >
        ...
        <div
          ...
          style={{
            borderRadius: "2rem",
            background:
              "radial-gradient(circle at 18% 12%, rgba(255,255,255,0.14) 0%, rgba(255,255,255,0.05) 30%, transparent 52%)",
            opacity: 0.18,
            mixBlendMode: "screen",
          }}
        />
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.04) 50%, transparent 100%)",
            opacity: 0.12,
          }}
        />
        <div
